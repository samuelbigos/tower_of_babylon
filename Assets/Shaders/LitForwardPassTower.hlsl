...
Varyings LitPassVertex(Attributes input)
{
    ...
    // Wrap XZ around the origin with a given radius.
    float width = 50.0f;
    float3 origin = float3(0.0, 0.0, 20.0);    
    float angle = world.x / width * PI * 2.0f;
    float3x3 rot = float3x3(cos(angle), 0, -sin(angle),
                            0, 1, 0,
                            sin(angle), 0, cos(angle));

    float y = world.y;
    world = mul(rot, world - float3(world.x, 0, 0) - origin) + origin;
    world.y = y;
    
    vertexInput.positionWS = world;
